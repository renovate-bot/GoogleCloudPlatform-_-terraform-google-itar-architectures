...
#This module will create a new service account for compute engine DMZ and ensure that your VM instance is not associated with the default GCP service account
...
resource "google_service_account" "service_account" {
  ...
  account_id   = "sa-${var.sa_prefix}${random_id.sa_sufix.dec}"
  project      = var.project_id
  display_name = "DMZ vm_service_account"
  description  = "service account for DMZ vm"
}
#This Updates the IAM policy to grant a role to a new member. The members to which roles are assigned here are either existing service account or new service account created for Compute Engine DMZ
